...
def valid_number(campo, rule):
    ...
    print(var)
    return var
def valid_resp(campo):
    while True:
        resposta = input(f"{campo} (s/n): ").lower()
...
def anamnese_on():
    ...
    # História médica
    doencas_cronicas = valid_resp("Você tem alguma doença crônica? ")
    cirurgias = valid_resp("Você já fez alguma cirurgia? ")
    alergias = valid_resp("Você tem alguma alergia? ")
    medicamentos = valid_resp("Você toma algum medicamento? ")
    fumar = valid_resp("Você fuma? ")
    beber = valid_resp("Você bebe? ")
    historico_familiar =valid_resp("Você tem algum histórico familiar de doenças? ")
    ...
    patient_symptoms = [queixa_principal, doencas_cronicas, cirurgias, alergias, medicamentos, fumar, beber,
                        historico_familiar]
...
def calculate_distance(latitude1, longitude1, latitude2, longitude2):
    ...
    distance_in_km = earth_radius_km * math.acos(
        ...
        math.cos(math.radians(latitude1)) * math.cos(math.radians(latitude2)) * math.cos(
            math.radians(longitude1 - longitude2))
    )
...
def get_hospital():
    ...
    try:
        anamnese_data = read_json('../json/anamnese.json')
        if not anamnese_data:
            print("Você precisa preencher a ficha de anamnese antes de escolher um hospital.")
            anamnese_on()
            return None

        # Se o usuário tem uma ficha de anamnese, continua com a escolha do hospital
